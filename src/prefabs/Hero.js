...
class IdleState extends State {
    ...
    execute(scene, hero) {
        ...
        const FKey = scene.keys.FKey //Bounding context so that we may use FKey in Hero
        ...
        if(Phaser.Input.Keyboard.JustDown(FKey)){ //If F-key is pressed, transition to the circular state
            this.stateMachine.transition("circular");
            return //Return is necessary to exit out of the execute() function
        }
...
class MoveState extends State {
    execute(scene, hero) {
        ...
        const { left, right, up, down, space, shift } = scene.keys
        const HKey = scene.keys.HKey
        const FKey = scene.keys.FKey //Also required for this state
...
class CircularState extends State {
    enter(scene, hero){
        //hero.setTint(0x0000FF) //For testing, should change character tint to blue
        hero.body.setVelocity(0); //Stop the body from moving
        hero.anims.play("circular-attack").once("animationcomplete", () => { //Once circular animation is complete, return to idle state
            this.stateMachine.transition("idle");
        });
    }
}